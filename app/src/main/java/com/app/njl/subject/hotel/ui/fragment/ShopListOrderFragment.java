...
import com.app.njl.subject.hotel.model.entity.shoplist.StoreListOrderBean;
 ...
 */
public class ShopListOrderFragment extends StoreListBaseFragment implements CommonView<StoreListOrderBean.MessageBean.ResultBean>, HttpListener<String> {
    ...
    QuickAdapter<StoreListOrderBean.MessageBean.ResultBean> adapter;
    ...
    private List<StoreListOrderBean.MessageBean.ResultBean> lists = new ArrayList<>();
    ...
    public ShopListOrderFragment() {

    }

    public ShopListOrderFragment(int sceneryId) {
    ...
    public void initView() {
        context = getActivity();
        adapter = new QuickAdapter<StoreListOrderBean.MessageBean.ResultBean>(context, R.layout.recommend_shop_list_item) {
            @Override
            protected void convert(BaseAdapterHelper helper, StoreListOrderBean.MessageBean.ResultBean shop) {
                String typeStr = "";
                String positionStr = "";
                if(mSort == 1) {
                ...
                helper.setText(R.id.name, shop.getShopName())
                        ...
                        .setText(R.id.number, shop.getFoodTotalNum() + "道菜")
                        .setText(R.id.price, "均价" + shop.getFoodAveragePrice() + "起")
...
//                initRemoteData();
        ...
        filterListView.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
                ...
                if(flag == 1) {
                    ...
                    mPriceTv.setText("均价排序");
                }else if(flag == 3) {
                    mSort = 3;
                    if(position == 0) {
                        mSortOrder = 0;
                        mPriceTv.setText("均价升序");
                    }else if(position == 1) {
                        mSortOrder = 1;
                        mPriceTv.setText("均价降序");
                    }
                    //还原除位置的其他设置
                    mRecommendTv.setText("推荐排序");
                    mGoodCommTv.setText("好评排序");
    ...
    private void replaceFragment(String name) {
        ...
        for (int j = 0; j < MainActivity.fragmentTags.size(); j++) {
            ...
            if(f != null && f.isAdded()) {
                fragmentTransaction.hide(ShopListOrderFragment.this);
            }
        }
        if (fragment.isAdded()) {
...
//        mPresenter.detachView();
    ...
    @Override
    public void loadSuccess(List<StoreListOrderBean.MessageBean.ResultBean> stores) {
        lists.addAll(stores);
    ...
    public void queryStoresByOptions(String shopName) {
        ...
        Request<String> request = new StringRequestImpl(Constants.BASE_PATH + "shopList/listRestaurant", RequestMethod.POST);
        request.add("resortId", mSceneryId);
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case HttpConstants.QUERY_STORES_BYOPTIONS:
                ...
                StoreListOrderBean bean = JsonEasy.toObject(result, StoreListOrderBean.class);
                if(bean == null) return;
                ...
                if(code == 1) {
                    ...
                    List<StoreListOrderBean.MessageBean.ResultBean> shops = bean.getMessage().getResult();
                    if(mPageNo == 1) {
                        lists.clear();
                        adapter.clearAll();
                    }
