...
import com.app.njl.subject.hotel.model.entity.shopdetails.ShopPlayTourGuideBean;
import com.app.njl.subject.hotel.model.entity.shopdetails.ShopPlayTourGuidePicsBean;
 ...
 */
public class ShopPlayShowTourGuideActivity extends AppCompatActivity implements View.OnClickListener, HttpListener<String> {
    ...
    private void loadData() {
        setStayLiveTime();
        int resortId = getIntent().getIntExtra("shopId", 1);
        int guideId = getIntent().getIntExtra("guideId", 1);
        queryDatasPicByOptions(resortId, guideId);
        queryDatasByOptions(resortId, guideId);
    }
    ...
    public void queryDatasPicByOptions(int resortId, int guideId) {
        Request<String> request = new StringRequestImpl(Constants.BASE_PATH + "shop/guidePic", RequestMethod.POST);
        request.add("shopId", resortId);
        request.add("guideId", guideId);
        CallServer.getRequestInstance().add(this, HttpConstants.QUERY_SHOPPLAY_TOURGUIDE_PIC_BYOPTIONS, request, this, true, true);
    }

    public void queryDatasByOptions(int resortId, int guideId) {
        Request<String> request = new StringRequestImpl(Constants.BASE_PATH + "shop/guideNote", RequestMethod.POST);
        request.add("shopId", resortId);
        request.add("guideId", guideId);
        CallServer.getRequestInstance().add(this, HttpConstants.QUERY_SHOPPLAY_TOURGUIDE_BYOPTIONS, request, this, true, true);
    }
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case HttpConstants.QUERY_SHOPPLAY_TOURGUIDE_BYOPTIONS:
                Log.i("result", "result get shop tourGuide:" + result);
                if(result == null) return;
                ShopPlayTourGuideBean playTourGuideBean = JsonEasy.toObject(result, ShopPlayTourGuideBean.class);
                if(playTourGuideBean == null) return;
                code = playTourGuideBean.getCode();
                if(code == 1) {
                    ShopPlayShowTourGuideActivity.DefRecyclerViewAdapter mAdapter = new ShopPlayShowTourGuideActivity.DefRecyclerViewAdapter(playTourGuideBean.getMessage());
                    mRecyclerView.setAdapter(mAdapter);
                }
                break;
            case HttpConstants.QUERY_SHOPPLAY_TOURGUIDE_PIC_BYOPTIONS:
                Log.i("result", "result get shop tourGuide pic:" + result);
                if(result == null) return;
                ShopPlayTourGuidePicsBean picsBean = JsonEasy.toObject(result, ShopPlayTourGuidePicsBean.class);
                if(picsBean == null) return;
                code = picsBean.getCode();
                if(code == 1) {
                    final List<ShopPlayTourGuidePicsBean.MessageBean> messageBeans = picsBean.getMessage();
                    ShopPlayShowTourGuideActivity.AutoPagerAdapter pagerAdapter = new ShopPlayShowTourGuideActivity.AutoPagerAdapter(messageBeans);
...
//                            Log.i("position", "viewPager onPageScrollStateChanged position:" + state);
    ...
    public class AutoPagerAdapter extends PagerAdapter {
        private List<ShopPlayTourGuidePicsBean.MessageBean> mMessageBeens;

        public AutoPagerAdapter(List<ShopPlayTourGuidePicsBean.MessageBean> messageBeens) {
...
//            item.setImageResource(mMessageBeens.get(position).getPicUrl());
    ...
    class DefRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
        ShopPlayTourGuideBean.MessageBean mMessageBean;
        public DefRecyclerViewAdapter(ShopPlayTourGuideBean.MessageBean messageBean) {
        ...
        public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            return new ShopPlayShowTourGuideActivity.DefViewHolder(view);
        }

        @Override
        public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
            ShopPlayShowTourGuideActivity.DefViewHolder viewHolder = (ShopPlayShowTourGuideActivity.DefViewHolder)holder;
