...
import com.app.njl.subject.hotel.model.entity.shoplist.StoreListPlayBean;
 ...
 */
public class ShopListPlayFragment extends StoreListBaseFragment implements CommonView<StoreListPlayBean.MessageBean.ResultBean>, View.OnClickListener, HttpListener<String> {
    ...
    QuickAdapter<StoreListPlayBean.MessageBean.ResultBean> adapter;
...
//    private String playItems[] = new String[] {"门票低价优先", "导游低价优先", "自助游低价优先"};
    ...
    private List<StoreListPlayBean.MessageBean.ResultBean> lists = new ArrayList<>();
    ...
    private int mSort = 0; //0：表示按照默认排序1： 按照推荐排序2：按照好评数排序3：门票低价优先4：导游低价优先5：农家自助游低价优先
    ...
    public void initView() {
        context = getActivity();
        adapter = new QuickAdapter<StoreListPlayBean.MessageBean.ResultBean>(context, R.layout.shoplistplay_list_item) {
            @Override
            protected void convert(BaseAdapterHelper helper, StoreListPlayBean.MessageBean.ResultBean shop) {
                String typeStr = "";
                String positionStr = "";
                if(mSort == 1) {
                ...
                String priceSpecialStr = "￥" + shop.getTravelSpecialLowestPrice() + "自助";
                if(shop.getTravelSpecialState() == 0) {
                    priceSpecialStr = "无自助服务";
                }
                String priceGuideStr = "￥" + shop.getTravelGuideLowestPrice() + "导游";
                if(shop.getTravelGuideState() == 0) {
                    priceGuideStr = "无导游服务";
                }
                String priceTicketStr = "￥" + shop.getTravelTicketLowestPrice() + "门票";
                if(shop.getTravelTicketState() == 0) {
                    priceTicketStr = "无门票服务";
                }

                helper.setText(R.id.name, shop.getShopName())
                        .setText(R.id.comment, typeStr)
                        .setText(R.id.position, positionStr)
                        .setText(R.id.specialPrice, priceSpecialStr)
                        .setText(R.id.guidePrice, priceGuideStr)
                        .setText(R.id.ticketPrice, priceTicketStr)
...
//        mPresenter.detachView();
    ...
    @Override
    public void loadSuccess(List<StoreListPlayBean.MessageBean.ResultBean> stores) {
        lists.addAll(stores);
    ...
    public void queryStoresByOptions(String shopName) {
        ...
        Request<String> request = new StringRequestImpl(Constants.BASE_PATH + "shopList/listTravel", RequestMethod.POST);
        request.add("resortId", mSceneryId);
    ...
    public void onSucceed(int what, Response<String> response) {
        ...
        switch (what) {
            case HttpConstants.QUERY_STORES_BYOPTIONS:
                KLog.i("result get store list by play:" + result);
                StoreListPlayBean bean = JsonEasy.toObject(result, StoreListPlayBean.class);
                if(bean == null) return;
                code = bean.getCode();
                if(code == 1) {
                    if(bean.getMessage() == null) return;
                    mTotalPage = bean.getMessage().getTotalNumber();
                    List<StoreListPlayBean.MessageBean.ResultBean> shops = bean.getMessage().getResult();
                    if(mPageNo == 1) {
                        lists.clear();
                        adapter.clearAll();
                    }
